...
const getMapData = () => {
  ...
  state.chartData.geoJson.features.forEach((geo, i) => {
    if (geo.properties.name) {
      ...
      let maxValue = proxy.randomNumber(
        chartData.maxData / 2,
        chartData.maxData
      );
      ...
      if (geo.properties.center) {
        if (topIndex.includes(i)) {
          let pin = {
            ...
            value: [
              geo.properties.center[0],
              geo.properties.center[1],
              maxValue,
            ],
  ...
  state.chartData.ambulanceData = AmbulanceLocation.value;
  processOption();
};

const movePoint = () => {
  timer.value = setInterval(() => {
    for (let i = 0; i < AmbulanceLocation.value.length; i++) {
      moveAmbulance(AmbulanceLocation.value[i]);
    }
...
const moveAmbulance = (ambulance) => {

    ambulance.value[0] += 0.003; // 向右移动

    ambulance.value[1] += (Math.random() > 0.5) ? 0.003 : -0.003;
...
onMounted(() => {
  httpGetMap("/static/maps/", "taiwan.json").then((res) => {
